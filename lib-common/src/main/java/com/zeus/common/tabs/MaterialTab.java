...
public class MaterialTab implements View.OnTouchListener {
    ...
    public MaterialTab(Context ctx,boolean hasIcon) {
        ...
        if(Build.VERSION.SDK_INT < Build.VERSION_CODES.ICE_CREAM_SANDWICH) {
            if(!hasIcon) {
                completeView = LayoutInflater.from(ctx).inflate(R.layout.common__tab, null);

                text = (TextView) completeView.findViewById(R.id.text);
            }
            else {
                completeView = LayoutInflater.from(ctx).inflate(R.layout.common__tab_icon, null);
            ...
            if (!hasIcon) {
                // if there is no icon
                completeView = LayoutInflater.from(ctx).inflate(R.layout.common__material_tab, null);

                text = (TextView) completeView.findViewById(R.id.text);
            } else {
                // with icon
                completeView = LayoutInflater.from(ctx).inflate(R.layout.common__material_tab_icon, null);
